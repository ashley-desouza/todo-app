...
describe("TaskForm Component", () => {
	...
	test("disables the submit button when input is whitespace only", async () => {
		const wrapper = mount(TaskForm, { localVue, store });
	...
	test("enables the submit button when input has text", async () => {
		const wrapper = mount(TaskForm, { localVue, store });
		const input = wrapper.find("input");
	...
	test("dispatches the createTask action with the trimmed title on submit", async () => {
		const wrapper = mount(TaskForm, { localVue, store });
		const input = wrapper.find("input");
		await input.setValue("  Buy groceries  ");
		...
		// Vuex action handlers receive (context, payload), so that the payload is the second argument.
	...
	test("persists the input if the submit fails", async () => {
